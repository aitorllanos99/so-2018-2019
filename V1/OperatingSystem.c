...
int OperatingSystem_LongTermScheduler() {
	...
	//Intento ej 4
 	// if(indexOfExecutableProgram > PROGRAMSMAXNUMBER){ ComputerSystem_DebugMessage(103,SHUTDOWN);}
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	if(processSize <0){return processSize;} //PROGRAMDOESNOTEXIST == -1 || PROGRAMNOTVALID == -2

	// Obtain the priority for the process
	priority=OperatingSystem_ObtainPriority(programFile);
	
	if(priority <0){ return priority; } //PROGRAMNOTVALID == -2

	// Obtain enough memory space
 	loadingPhysicalAddress=OperatingSystem_ObtainMainMemory(processSize, PID);

	if(loadingPhysicalAddress < 0){ return loadingPhysicalAddress;} //TOOBIGPROCESS == -4

	int direccionMem = OperatingSystem_LoadProgram(programFile, loadingPhysicalAddress, processSize);
	if(direccionMem < 0){return direccionMem;} //TOOBIGPROCESS == -4
...
void OperatingSystem_MoveToTheREADYState(int PID) {
	...
	//ComputerSystem_DebugMessage(110,SYSPROC,PID,executableProgram->executableName,statesNames[1]);
...
void OperatingSystem_PrintReadyToRunQueue(){
	...
	for(i=0; i<numberOfReadyToRunProcesses;i++){
		if(i != 0){
	 	ComputerSystem_DebugMessage(107, SHORTTERMSCHEDULE,readyToRunQueue[i],processTable[readyToRunQueue[i]].priority);
	 }
	}
}