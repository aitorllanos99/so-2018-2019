...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram)
{
    ...
    if (memoryadd < 0) // TOOBIGPROCESS == -4
    {
        return memoryadd;
    }
...
int OperatingSystem_ObtainMainMemory(int processSize, char *n, int PID)
{
    ...
    ComputerSystem_DebugMessage(142, SYSMEM, PID, n, processSize);
    ...
    int i = 0;
    int finalPartition = NOPROCESS;
    int partitionSize = MAINMEMORYSIZE;
...
//Checks if the partition can be solved
int OperatingSystem_PartitionLooker(int ps)
{
    int i = PARTITIONTABLEMAXSIZE - 1;
    int topPartition = 0;
    int numberOfPartitions = 0;

    while (PARTITIONTABLEMAXSIZE >= i && i >= 0)
    {
...
void OperatingSystem_GiveMemory(int PID, char *executableName, int selectedPartition)
{
    ...
    OperatingSystem_ShowTime(SYSMEM);
    ComputerSystem_DebugMessage(143, SYSMEM, selectedPartition, partitionsTable[selectedPartition].initAddress, partitionsTable[selectedPartition].size, PID, executableName);
...
void OperatingSystem_HandleException()
{
    ...
    ComputerSystem_DebugMessage(140, INTERRUPT, executingProcessID, name, reason);
...
void OperatingSystem_HandleSystemCall()
{
    ...
    default:
        ...
        ComputerSystem_DebugMessage(141, INTERRUPT, executingProcessID, processTable[executingProcessID].nombre, systemCallID);
    ...
    if (numWakeUpProcess > 0 || newProcess > 0)
    {
        ...
        if (processTable[executingProcessID].queueID == DAEMONSQUEUE)
        {
            if (numberOfReadyToRunProcesses[USERPROCESSQUEUE] > 0)
            {
                ...
                selectedProcess = OperatingSystem_ShortTermScheduler();
                OperatingSystem_ShowTime(SHORTTERMSCHEDULE);
                ComputerSystem_DebugMessage(121, SHORTTERMSCHEDULE, copyOfExecutingProcessID, processTable[copyOfExecutingProcessID].nombre, selectedProcess, processTable[selectedProcess].nombre);
                OperatingSystem_PreemptRunningProcess();
                OperatingSystem_Dispatch(selectedProcess);
                OperatingSystem_PrintStatus();
            }
...
void OperatingSystem_FreeMainMemory()
{
    ...
    while (i < PARTITIONTABLEMAXSIZE)
    {
        if (partitionsTable[i].PID == executingProcessID && partitionsTable[i].occupied == 1)
        {
            ...
            OperatingSystem_ShowTime(SYSMEM);
            ComputerSystem_DebugMessage(145, SYSMEM, i, partitionsTable[i].initAddress, partitionsTable[i].size, executingProcessID, processTable[executingProcessID].nombre);
